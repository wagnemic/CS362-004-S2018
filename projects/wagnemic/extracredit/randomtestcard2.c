...
int compareGameStatesRemodel(struct gameState *gameBefore, struct gameState *gameAfter, int remodelFailed, int trashedCard, int gainedCard)
{
...
int runOneRandomTestForRemodelEffect()
{
    ...
    if (gameBefore.handCount[PLAYER] == 1)
    {
        ...
        if (1 == compareGameStatesRemodel(&gameBefore, &gameAfter, 0, -1, -1)) return 1; // the -1s indicate remodel discarded and nothing was trashed or gained
    ...
    else if (handPosToRemodel == remodelHandPos)
    {
        ...
        if (1 == compareGameStatesRemodel(&gameBefore, &gameAfter, 1, -1, -1)) return 1; // last 2 args are not used when the 3rd to last is 1
    ...
    else
    {
        ...
        if (1 == compareGameStatesRemodel(&gameBefore, &gameAfter, 0, gameBefore.hand[PLAYER][handPosToRemodel], supplyPosToGain)) return 1;
        cardEffectReturnExpected = 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    //for (i = 1; i <= 1000; ++i)
    //{
        return runOneRandomTestForRemodelEffect();
    //}
