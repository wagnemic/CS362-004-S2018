 ...
 * this program can run 48 unit tests (so give it an integer 1-48, inclusive)
...
int main(int argc, char *argv[])
{
    ...
    int curTest = 1; // increment this from 1-48 as we go thru test cases and once we reach the one we want (chosenTestCase) return appropriate test case result value
    
    if (chosenTestCase < 1 || chosenTestCase > 48)
        return 2;
    ...
    for (s = 0; s < 2; ++s)
    {
        // only run initializeGame one time per test case, so only if s is 0 and our test case is 1-24, or when s=1 and test case is 25-48
        if (s == 0 && chosenTestCase <= 24) 
        {
        ...
        for (v = 0; v < 2; ++v)
        {
            if (curTest == chosenTestCase) 
            {
                ...
                if (r == isGameOverExpectedResult[v] && memcmp(&testGame, &holdGame, sizeof(struct gameState)) == 0)
                    return 0;
                else
                    return 1;
            }
        ...
        {   
            if (curTest == chosenTestCase) 
            {
                ...
                if (n >= 3)
                    if (r == 1 && memcmp(&testGame, &holdGame, sizeof(struct gameState)) == 0)
                        return 0;
                    else
                        return 1;
                else
                    if (r == 0 && memcmp(&testGame, &holdGame, sizeof(struct gameState)) == 0)
                        return 0;
                    else
                        return 1;
            }
        ...
        {      
            if (curTest == chosenTestCase) 
            {
                ...
                r = isGameOver(&testGame);
                    ...
                    if (r == 1 && memcmp(&testGame, &holdGame, sizeof(struct gameState)) == 0) // should always be a game over with any 3 empty supplies
                        return 0;
                    else
                        return 1;
            }
